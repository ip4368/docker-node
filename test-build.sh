...
for version in "${versions[@]}"; do
  ...
  info "Building $tag..."
  docker build -t ip4368/node-arm64:$tag $version
  ...
  OUTPUT=$(docker run --rm -it ip4368/node-arm64:$tag node -e "process.stdout.write(process.versions.node)")
  if [ "$OUTPUT" != "$tag" ]; then
  ...
  for variant in $variants; do
    ...
    docker build -t ip4368/node-arm64:$tag-$variant $version/$variant
    ...
    OUTPUT=$(docker run --rm -it ip4368/node-arm64:$tag-$variant node -e "process.stdout.write(process.versions.node)")
    if [ "$OUTPUT" != "$tag" ]; then
